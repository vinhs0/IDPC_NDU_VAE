...
class GA:
    ...
    def vector_to_individual(self, vector: np.array) -> Individual:
        ...
        # Mapping Logic:
        # Scale the continuous range [-1, 1] to the discrete Integer range [1, Num_Domains]
        min_node = 1
    ...
    def run(self, seed: int) -> Individual:
        ...
        with open(full_path, "w") as fw_gen:
            ...
            while generation < Configs.MAX_GENERATIONS:
                ...
                # ============================================================
                # 4. HMQD KNOWLEDGE TRANSFER (Triggered every 100 Epochs)
                # ============================================================
                # Check: generation is 100, 200, 300... (and not 0)
                if generation > 0 and generation % transfer_interval == 0:
                    ...
                    if valid_inds > 0:
                        ...
                        # B. Train VAE (Re-initialize or Online Update)
                        # We re-init here to avoid over-fitting old data, 
                        # but you can also make self.vae persistent.
                        ...
                        for method_idx, new_vec in new_solution_data:
                            ...
                            # Note: "New Grid" reward requires a Behavior Descriptor map, 
                            # which isn't in your basic GA class, so we skip +2 reward here.
                        ...
                        # E. Merge & Reselect
                        # We inject these new solutions into the population immediately
                        # and run survival selection AGAIN to keep population size constant.
